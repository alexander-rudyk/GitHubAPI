...
class CreateGistViewController: XLFormViewController {
    ...
    override func viewDidLoad() {
        ...
        self.navigationItem.leftBarButtonItem = UIBarButtonItem(barButtonSystemItem: .cancel,
                                                                target: self,
                                                                action: #selector(canselPressed(_:)))
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(barButtonSystemItem: .save,
                                                                 target: self,
                                                                 action: #selector(savePresed(_:)))
    }
    
    func canselPressed(_ sender: UIBarButtonItem) {
        let _ = self.navigationController?.popViewController(animated: true)
    }
    
    func savePresed(_ sender: UIBarButtonItem) {
        ...
        GitHubAPIManager.shared.createNewGist(description: description,
                                              isPublic: isPublic,
                                              files: files)
        { result in
    ...
    private func initializeForm() {
        ...
        let descriptionRow = XLFormRowDescriptor(tag: "description",
                                                 rowType: XLFormRowDescriptorTypeText,
                                                 title: "Description")
        descriptionRow.isRequired = true
        section1.addFormRow(descriptionRow)
        
        let isPublicRow = XLFormRowDescriptor(tag: "isPublic",
        ...
        let filename = XLFormRowDescriptor(tag: "filename",
                                           rowType: XLFormRowDescriptorTypeText,
                                           title: "Filename")
        filename.isRequired = true
        section2.addFormRow(filename)
        
        let fileContent = XLFormRowDescriptor(tag: "fileContent",
